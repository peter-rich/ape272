...
int main(int argc,char**argv){
  ...
  // SAFETY GUARD: the "star pruning" step in expand() is only sound when
  // THR >= B(N) := C(N,2) + 1 + floor((N-1)/4), the exact max size of a
  // starred family (Theorem 1.4 of the paper). Below that threshold the
  // pruning silently discards valid witnesses and the decision "NO clique
  // > THR" becomes UNSOUND (false negative). We verified this empirically:
  // for N=6, THR=16 (< B(6)=17) the solver wrongly reports "NO clique > 16"
  // even though a clique of size 17 exists.
  {
    long Bn = (long)N * (N - 1) / 2 + 1 + (N - 1) / 4;
    if ((long)THR < Bn) {
      fprintf(stderr,
        "REFUSING TO RUN: THR=%d is below B(N)=%ld (exact max starred-family size).\n"
        "The star-pruning shortcut in expand() is unsound for THR < B(N) and can\n"
        "silently produce a false \"NO clique > THR\" answer. Re-run with THR >= %ld.\n",
        THR, Bn, Bn);
      return 1;
    }
  }
